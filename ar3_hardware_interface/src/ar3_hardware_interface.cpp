...
AR3HardwareInterface::AR3HardwareInterface(ros::NodeHandle & nh) : nh_(nh)
{
  ...
  {
    if (!nh_.getParam(
          "/ar3/hardware_driver/encoder_steps_per_deg/" + joint_names_[i], enc_steps_per_deg[i]))
    {
      ROS_WARN("Failed to get params for /ar3/hardware_driver/encoder_steps_per_deg/");
    }

    if (!nh_.getParam(
          "/ar3/hardware_interface/joint_offsets/" + joint_names_[i], joint_offsets_[i]))
    {
  ...
  for (int i = 0; i < num_joints_; ++i)
  {
    ...
    nh_.getParam(
      "/ar3/joint_limits/" + joint_names_[i] + "/max_acceleration", acceleration_limits_[i]);
...
double AR3HardwareInterface::degToRad(double deg) { return deg / 180.0 * M_PI; }

double AR3HardwareInterface::radToDeg(double rad) { return rad / M_PI * 180.0; }
